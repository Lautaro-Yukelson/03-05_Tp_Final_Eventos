...
export default class EventService {
  ...
  async getEnrollments(id, query) {
    try {
      ...
      return response.length > 0
        ? [{ success: true, response: response }, 200]
        : [
            { success: false, message: 'No hay enrollments para ese evento' },
            404,
          ];
    } catch (error) {
        ...
        return [
          {
            success: false,
            message:
              'El nombre y la descripción deben tener al menos tres letras',
          },
          400,
        ];
      }
        ...
        return [
          {
            success: false,
            message:
              'El precio y la duración en minutos no pueden ser valores negativos',
          },
          400,
        ];
      }
  ...
  async updateEvent({ body }) {
    ...
    try {
      ...
      return [
        { success: true, message: 'Evento actualizado correctamente' },
        200,
      ];
    } catch (error) {
