...
router.get('/', async (req, res) => {
	try {
		const [response, status] = await svc.getUsers();
...
router.get('/login', async (req, res) => {
	try {
		...
		return res.status(status).json({ success: true, response: response });
	} catch (error) {
		console.error('Controller error - .get("/login") : ', error);
		return res
			.status(500)
			.send({ success: false, message: 'Controller error - .get("/login"' });
	}
});
